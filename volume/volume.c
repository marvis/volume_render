...
GLubyte *
loadtex3d(char* imgfile, int *texwid, int *texht, int *texdepth, int *texcomps)
{
    //char *filename;
    GLubyte *tex3ddata;
	...
	unsigned char* img = readtiff(imgfile , texwid, texht, texdepth, texcomps);
	int size = (*texwid) * (*texht) * (*texdepth);
	...
	*/
	...
	for(i = 0; i < size; i++)
	{
		...
		{
			tex3ddata[4*i] = img[i];
			tex3ddata[4*i+1] = img[i];
			tex3ddata[4*i+2] = img[i];
			tex3ddata[4*i+3] = 0xff;
		}
		else if(*texcomps == 3)
		{
			//printf("texcomps = %d\n", *texcomps);
			tex3ddata[4*i] = img[3*i];
			tex3ddata[4*i+1] = img[3*i+1];
			tex3ddata[4*i+2] = img[3*i+2];
			tex3ddata[4*i+3] = 0xff;
		}
	}
	free(img);
...
main(int argc, char *argv[])
{
    ...
    tex3ddata = loadtex3d(argv[1], &texwid, &texht, &texdepth, &texcomps);
