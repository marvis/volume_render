...

{
	...
	if(tif)
	{
		unsigned char *img = (unsigned char*)malloc((*channels)*(*width)*(*height)*(*depth)* sizeof(unsigned char));
		int layer = 0;
		do
		{			
			...
			unsigned char *buf = (unsigned char*)malloc(linebytes * sizeof(unsigned char));
...
//===================================================================================================
{
    ...
    if (tif)
    {
		...
		for(layerIndex = 0; layerIndex < depth ; layerIndex++)
		{
			...
			//create a dummy array
			int linebytes = channels*width;
			unsigned char *buf = (unsigned char*) malloc(sizeof(unsigned char) * linebytes);
