...
// Functions:
          ...
          case   cInt: Stack[SP] = (float)floor(Stack[SP]+.5); break;
          case   cLog: if(Stack[SP] <= 0) { evalErrorType=3; return 0; }
                       Stack[SP] = (float)log(Stack[SP]); break;
          case cLog10: if(Stack[SP] <= 0) { evalErrorType=3; return 0; }
                       Stack[SP] = (float)log10(Stack[SP]); break;
...
private:
    ...
    {
        value = orig;
        if(IsInverted()) { value = 1.0f / value;
        ...
        Redo:
            for(pit a=GetBegin(); a!=GetEnd(); ++a)
            {
                ...
                if(poslist.size() >= min)
                {
                    ...
                    float factor = (float)(poslist.size() + 1);
